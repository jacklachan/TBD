...
| `Scenario` | `schema_version`, `scenario_id`, `scenario_version`, `seed`, `epoch_utc`, `horizon_s`, at least three `objects` (the debris-stream fixture carries 14; the `two_operators` fixture has a second manoeuvrable object with an optional `operator` label), `satellite_id`, `primary_threat_id`, `known_windows: list[BurnWindow]`, `provenance`; all IDs resolve; exactly one object maneuverable |
