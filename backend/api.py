    ...
    def export(
        ...
        if format == "cdm":
            return PlainTextResponse(
                _cdm_report(snapshot), media_type="text/plain; charset=utf-8"
            )
        return PlainTextResponse(
...
def _cdm_report(snapshot: dict) -> str:
    """A CCSDS-shaped Conjunction Data Message for the screened encounters.

    Coordinating orbital traffic is only partly a question of who talks to whom;
    the substance is whether one operator's result means anything to another.
    The CDM is the format that question already has an answer in, so the
    decision leaves here in it rather than in a shape only this tool reads.

    This is CDM-*shaped*, and the COMMENT lines say so. A conforming CDM carries
    a covariance for each object, and public element sets do not have one. What
    is emitted is what was actually computed: the screened objects, the times,
    the separations and the frame they were computed in.
    """
    ...
    lines.append("COMMENT Generated by Orion West. NOT a conforming CDM:")
    lines.append("COMMENT no covariance is available from public element sets,")
    lines.append("COMMENT so no collision probability is stated anywhere below.")
    lines.append(
        f"COMMENT Conjunction geometry is synthetic; the primary object orbit is"
    )
    lines.append(
        f"COMMENT seeded from NORAD {provenance.get('norad_id')} at its own epoch."
    )
    lines.append(f"CREATION_DATE = {_now()}")
    lines.append("ORIGINATOR = ORION WEST")
    lines.append(f"MESSAGE_ID = {snapshot['case_id']}")
    lines.append("")
    ...
    lines.append(f"COMMENT Screening result: {chosen['status']}")
    lines.append(f"COMMENT Candidate: {chosen['candidate_id']}")
    lines.append(f"COMMENT Evidence: {chosen['validation_id']}")
    lines.append(
    ...
    for encounter in sorted(chosen["encounters"], key=lambda e: e["min_separation_m"]):
        ...
        lines.append("COMMENT ---- relative metadata ----")
        lines.append(f"TCA = {scenario['epoch_utc']} + {encounter['tca_s']:.3f} [s]")
        lines.append(f"MISS_DISTANCE = {encounter['min_separation_m']:.3f} [m]")
        lines.append(
            f"RELATIVE_SPEED = {encounter['relative_speed_mps']:.3f} [m/s]"
        )
        lines.append(f"COMMENT BOUNDARY_KIND = {encounter['boundary_kind']}")
        ...
        for role, object_id in (("OBJECT1", satellite_id), ("OBJECT2", secondary_id)):
            ...
            lines.append(f"OBJECT = {role}")
            lines.append(f"OBJECT_DESIGNATOR = {object_id}")
            lines.append(
                "CATALOG_NAME = CELESTRAK GP"
                if role == "OBJECT1"
                else "CATALOG_NAME = SYNTHETIC"
            )
            lines.append(f"OBJECT_NAME = {entry.get('name', object_id)}")
            lines.append(f"EPHEMERIS_NAME = {provenance.get('model_version')}")
            lines.append(
                f"MANEUVERABLE = {'YES' if entry.get('kind') == 'SATELLITE' else 'NO'}"
            )
            lines.append(f"REF_FRAME = {provenance.get('frame')}")
            lines.append("COVARIANCE_METHOD = NONE")
            lines.append("")

    execution = snapshot.get("execution")
    if execution:
        lines.append("COMMENT ---- manoeuvre ----")
        lines.append(f"COMMENT EXECUTION_ID = {execution['execution_id']}")
        lines.append(f"COMMENT CANDIDATE = {execution['candidate_id']}")
        lines.append(f"COMMENT EXECUTED_AT = {execution['executed_at_utc']}")
        lines.append(f"COMMENT MODE = {execution['mode']} (nothing was transmitted)")
        lines.append("")
